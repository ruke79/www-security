...
curl http://localhost:19080/api/ch2/public

# Register a new user (BCrypt-hashed, salted storage)
curl -X POST http://localhost:19080/api/ch2/register \
  -H 'Content-Type: application/json' \
  -d '{"username":"bob","password":"bobpassword"}'

# Admin-only resource via the PDP - PERMIT for admin, DENY for alice
curl -u admin:admin123 http://localhost:19080/api/ch2/admin-resource
curl -u alice:alice123 http://localhost:19080/api/ch2/admin-resource   # expect 403

# Hammer the rate limiter (default: 20 burst, 5/sec refill) to see a 429
for i in $(seq 1 30); do curl -s -o /dev/null -w "%{http_code}\n" http://localhost:19080/api/ch2/public; done
```
...
```bash
# --- Basic ---
curl -u admin:admin123 http://localhost:19080/api/ch3/basic/recipe
curl -u alice:alice123 -X POST http://localhost:19080/api/ch3/basic/recipe \
...
curl --digest -u prabath:prabath123 http://localhost:19080/api/ch3/digest/recipe
curl --digest -u alice:alice123 http://localhost:19080/api/ch3/digest/recipe/10000
```
...
TOKEN=$(curl -s -u lucidchart-client:lucidchart-secret -X POST http://localhost:19080/oauth2/token \
...
curl -H "Authorization: Bearer $TOKEN" http://localhost:19080/api/ch5/foo-api/drive

# 3. Brokered delegation: exchange it for a narrower, snapfish-scoped token
EXCHANGED=$(curl -s -H "Authorization: Bearer $TOKEN" -X POST http://localhost:19080/api/ch5/broker/exchange \
  -H 'Content-Type: application/json' \
...
curl -H "Authorization: Bearer $EXCHANGED" http://localhost:19080/api/ch5/snapfish-api/print
curl -H "Authorization: Bearer $TOKEN" http://localhost:19080/api/ch5/snapfish-api/print   # expect 403
```
...
```bash
# --- Client Credentials ---
curl -u demo-service-client:service-secret -X POST http://localhost:19080/oauth2/token \
...
curl -H "Authorization: Bearer $TOKEN" http://localhost:19080/api/ch7/resource
...
#   http://localhost:19080/oauth2/authorize?response_type=code&client_id=demo-authcode-client
#     &redirect_uri=http://127.0.0.1:19080/authorized&scope=ch7.read&code_challenge=<challenge>&code_challenge_method=S256
...
curl -H "Authorization: Bearer $TOKEN" -H "DPoP: $PROOF2" http://localhost:19080/api/ch8/protected/resource   # expect 401 (jti replay)
```
...
curl -H "Authorization: Bearer $TOKEN" http://localhost:19080/api/ch9/resource

# Introspection (auto-provided by Spring Authorization Server)
curl -u demo-jwtbearer-client:jwtbearer-secret -X POST http://localhost:19080/oauth2/introspect -d token="$TOKEN"

# Revocation
curl -u demo-jwtbearer-client:jwtbearer-secret -X POST http://localhost:19080/oauth2/revoke -d token="$TOKEN"
curl -H "Authorization: Bearer $TOKEN" http://localhost:19080/api/ch9/resource   # still valid until it expires - JWTs aren't invalidated locally by revocation, only via introspection lookups
```
...
curl -i http://localhost:19080/api/ch10/resource/photo-42

# 2. Exchange that ticket for an RPT (Requesting Party Token)
TICKET=<paste the "ticket" value from step 1>
RPT=$(curl -s -X POST http://localhost:19080/api/ch10/uma/token -H 'Content-Type: application/json' \
  -d "{\"grant_type\":\"urn:ietf:params:oauth:grant-type:uma-ticket\", \"ticket\":\"$TICKET\"}" \
  | python3 -c "import sys,json;print(json.load(sys.stdin)['access_token'])")

# 3. Retry with the RPT
curl -H "Authorization: Bearer $RPT" http://localhost:19080/api/ch10/resource/photo-42

# A ticket only works once
curl -X POST http://localhost:19080/api/ch10/uma/token -H 'Content-Type: application/json' \
...
curl "http://localhost:19080/api/ch11/external-idp/assertion?user=alice@foo-inc.example"
...
curl -s -X POST http://localhost:19080/api/ch6/signature/base-string \
  -H 'Content-Type: application/json' \
...
curl -s -X POST http://localhost:19080/api/ch6/signature/hmac-sha1 \
  -H 'Content-Type: application/json' \
  -d '{"httpMethod":"POST","baseUri":"http://server.com/oauth/request-token",
...
curl -i -X POST http://localhost:19080/api/ch6/nonce/check \
...
curl -s -X POST http://localhost:19080/api/ch9b/register -H 'Content-Type: application/json' \
...
curl -s -o /dev/null -w "%{http_code}\n" -X POST http://localhost:19080/api/ch12/id-token/validate \
...
curl -s -X POST http://localhost:19080/api/ch12/userinfo -H 'Content-Type: application/json' \
  -d "{\"idToken\":\"$IDT\"}"
```
...
JWS=$(curl -s -X POST http://localhost:19080/api/ch13/jws/hmac/sign -H 'Content-Type: application/json' \
  -d "{\"secret\":\"$SECRET\",\"subject\":\"alice\"}" | python3 -c "import sys,json;print(json.load(sys.stdin)['jws'])")
curl -s -X POST http://localhost:19080/api/ch13/jws/hmac/verify -H 'Content-Type: application/json' \
  -d "{\"jwt\":\"$JWS\",\"secret\":\"$SECRET\"}"   # {"valid":true,...}; a wrong secret -> valid:false

# JWS with RSA-SHA256 (server keypair)
RJWS=$(curl -s -X POST http://localhost:19080/api/ch13/jws/rsa/sign -H 'Content-Type: application/json' \
